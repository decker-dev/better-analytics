...
const CHART_TYPES = {
  area: {
    value: "area" as const,
    label: "Area",
    component: Area as React.ComponentType<{
      dataKey: string;
      stroke: string;
      fill: string;
      type: "monotone";
  ...
  bar: {
    value: "bar" as const,
    label: "Bar",
    component: Bar as React.ComponentType<{
      dataKey: string;
      stroke: string;
      fill: string;
      type: "monotone";
...
const chartUtils = {
  ...
  generateConfig: (aggregatorConfig: Record<string, unknown>): ChartConfig =>
  ...
  const grouped = items.reduce(
    (acc, item) => {
      ...
      if (!acc[formattedDate]) {
        acc[formattedDate] = { dateLabel: formattedDate };
      }

      const existing = acc[formattedDate];
      for (const [key, fn] of Object.entries(aggregator)) {
        existing[key] = ((existing[key] as number) || 0) + fn(item);
      }
        ...
        <PopoverTrigger asChild>
          ...
          >
            <span className="truncate">
              {DATE_FORMATS.find((f) => f.value === selectedFormat)?.label ||
                selectedFormat}
            </span>
...
function isValidDataField<T extends object>(
  data: T[],
  field: keyof T,
): boolean {
  return data.length > 0 && !!data[0] && field in data[0];
}
...
function useChartInteraction<TData>({
  ...
  const handleMouseDown = (e: { activeLabel?: string; chartX?: number }) => {
    if (e.activeLabel) {
      setRefAreaLeft(e.activeLabel);
      setRefAreaRight(e.activeLabel);
      setInitialX(e.chartX ?? null);
      setIsSelecting(true);
    }
  };

  const handleMouseMove = (e: { activeLabel?: string; chartX?: number }) => {
    if (
      isSelecting &&
      e.activeLabel &&
      initialX !== null &&
